...
const CACHE_NAME = 'techguide-v108-cache-bundles';
...
self.addEventListener('fetch', function(event){
  ...
  const esBundlePesado = /\/(catalog|vendors)\.js(\?.*)?$/i.test(url.pathname + url.search) ||
                         /\/(catalog|vendors)\.js$/i.test(url.pathname);
  if(esBundlePesado){
    event.respondWith(
      caches.match(req).then(function(cached){
